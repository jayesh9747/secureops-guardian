...
ANALYSIS_ONLY sandbox hard stop: never call exec, create a sandbox, inspect a sandbox filesystem, materialize a skill, or use a sandbox to recover, transform, or re-fetch evidence. A blob or download reference is not complete file content. Use only bounded official GitHub MCP reads to obtain or exactly reconstruct required bytes; if they remain unavailable, return INCONCLUSIVE immediately. Any sandbox or exec event invalidates an ANALYSIS_ONLY result.
...
For a completed FindingPack analysis, use terminal status FINDINGS when one or more deterministic findings exist and NO_DETERMINISTIC_FINDING when none exist. Use NO_DETERMINISTIC_FINDING, never NO_FINDING, CLEAN, PASS, or another invented alias, in the Incident Brief and run receipt. A no-finding sentence must remain scoped to the exact selected pack, documented static subset, file, and revision: say only that no deterministic finding from that pack was detected. Never broaden it to no vulnerabilities, secure, safe, clean, compliant, or another general assurance.
...
For K8S-WORKLOAD-003, emit one finding for Pod-level runAsUser 0 at the exact Pod field and one finding for each container-level runAsUser 0 at its exact container field. A container override does not suppress the prohibited Pod-level field. Include runAsNonRoot from the same scope, or the effective Pod value for a container finding, as observed context; never emit a separate runAsNonRoot finding. For K8S-WORKLOAD-004, emit one finding for missing drop ALL and one finding for each unsafe added capability; NET_BIND_SERVICE is the only allowed added capability in this bounded Restricted subset. The missing-drop finding's JSONPath must end in capabilities.drop, never the parent capabilities path; each unsafe addition must use its exact capabilities.add array index.
...
For workload analysis, use direct official GitHub MCP reads only. If get_file_contents returns a blob or download reference instead of inline bytes, the only permitted recovery is bounded commit-history evidence: list the target path at the suspect revision with perPage 2, read the immediate parent with get_commit full_patch, and reconstruct the suspect postimage only when the parent addition and suspect patch determine every byte exactly. Verify the reconstructed Git blob SHA against the official file result. If those reads do not return or determine the exact patch and complete blob content required by the pack, return INCONCLUSIVE; never call exec or create a sandbox to recover, transform, or re-fetch workload evidence.
...
OpenUI syntax gate: inside the single openui fence, the first nonblank statement must begin with root = Stack( and every later statement must use the assignment-based OpenUI DSL returned by get_openui_instructions. Never emit JSX, HTML, React components, or Tailwind classes; never use angle-bracket component syntax or className. If a valid DSL program cannot be produced, return the complete Markdown recovery rendering instead.
