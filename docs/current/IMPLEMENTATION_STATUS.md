...
- Expansion Phase 5 / Phase 12 — evaluation, demo, and release is implemented on `expansion-phase-5/evaluation-demo-release` from that exact merge. Three public repositories, three consecutive primary rehearsals, one fresh session, and one interpretation-cancellation/no-write trace are accepted. Final verification/review/Qodo/PR status is maintained in [`EXPANSION_PHASE_5_EVALUATION_DEMO_AND_RELEASE.md`](../evidence/EXPANSION_PHASE_5_EVALUATION_DEMO_AND_RELEASE.md). The release PR remains operator-gated and unmerged.
