...
The remediation support gate internally selects exactly `k8s-network-egress-v1`. The registered `guardian-network-egress-v1` TrueForge skill is pinned to immutable source revision `guardian-network-egress-v1.0.2`, source commit `bdea775220c07c20bd7f433cb3e12793d105b266`, and manifest digest `cf91d512c8be9939add2cb7f5700151164a1953a7864c1a77708b3d706756aaf`. Users do not name or attach verifier files. The old five-filename exact-JSON envelope is accepted only as a deprecated compatibility shape; it cannot select a file or pack. `ANALYSIS_ONLY` never enters Daytona, so the attached skill is not materialized into a sandbox.
