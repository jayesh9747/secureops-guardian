...
The remediation support gate internally selects exactly `k8s-network-egress-v1`. The registered `guardian-network-egress-v1` TrueForge skill is pinned to immutable source revision `guardian-network-egress-v1.0.0` and manifest digest `4f11fdc732b3aa49361fe1076949986a890dfa070d0f3853780029d7bab7df40`. Users do not name or attach verifier files. The old five-filename exact-JSON envelope is accepted only as a deprecated compatibility shape; it cannot select a file or pack. `ANALYSIS_ONLY` never loads or materializes the skill.
