...
export interface VerifierCliOptions {
  ...
  requiredPackRoot: string;
  emit: (value: unknown) => void;
}

class CliArgumentError extends Error {}

function parseArguments(argumentsList: string[], requiredPackRoot: string): CliArguments {
  ...
  if (packRoot !== requiredPackRoot) {
    throw new CliArgumentError(`--pack-root must be exactly ${requiredPackRoot}.`);
  }
...
export async function runVerifierCli(options: VerifierCliOptions): Promise<number> {
  const { argumentsList, requiredPackRoot, emit } = options;
  let cli: CliArguments;
  try {
    cli = parseArguments(argumentsList, requiredPackRoot);
  } catch (error) {
    emit({
      schema_version: 1,
